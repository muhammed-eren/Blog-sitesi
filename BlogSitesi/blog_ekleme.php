...
if(isset($_POST["submit"])) {
    ...
    $baslik = $_POST['baslik'];
    $yazi = $_POST['yazi'];
    // $_SESSION['username'] değerini alın
    ...
    if (!empty($_FILES["resim"]["tmp_name"])) { // Dosya seçilmiş mi kontrol et
        ...
        if ($check !== false) {
            ...
            if (move_uploaded_file($_FILES["resim"]["tmp_name"], $targetFile)) {
                ...
                $sql = "INSERT INTO bloglar (resim, baslik, kategori, blogYazi, paylasan) VALUES ('$resim', '$baslik', '$kategori', '$yazi', '$id')";
        ...
        $sql = "INSERT INTO bloglar (baslik, kategori, blogYazi, paylasan) VALUES ('$baslik', '$kategori', '$yazi', '$id')";
        // SQL sorgusunu çalıştırın
        $db->query($sql);
        if ($sql) {
            ?>
